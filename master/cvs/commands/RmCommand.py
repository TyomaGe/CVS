            ...
            if args.cached:
                ...
                for tracked_path in tracked_files:
                    if (tracked_path == relative_path or
                        ...
                        matched = True
                        index_handler.restore([tracked_path])
                if not matched:
                ...
                if self.__path_handler.exists(abs_path):
                    if self.__path_handler.is_dir(abs_path):
                        for root, _, files in self.__path_handler.walk(
                                abs_path):
                            for f in files:
                                full_path = self.__path_handler.make_path(
                                    root,
                                    f
                                )
                    ...
                    if not matched:
                        print(
                            f"\033[91m'{relative_path}' is not tracked\033[0m")
