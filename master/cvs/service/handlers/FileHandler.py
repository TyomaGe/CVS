...
class FileHandler:
    def __init__(self, dir_path, cvs_dir):
        self.__index_handler = IndexFileHandler(cvs_dir)
        self.__dir = dir_path
    ...
    def remove_dir(self, abs_dir_path):
        for root, _, files in self.__path_handler.walk(abs_dir_path):
            for f in files:
                ...
                self.__remove_if_tracked(rel_path, full_path)
        self.__path_handler.remove_empty_dirs_recursive(abs_dir_path)

    def remove_file(self, relative_path, abs_path):
        self.__remove_if_tracked(relative_path, abs_path)
        self.__path_handler.remove_empty_dirs_recursive(
            self.__path_handler.get_dirname(abs_path)
        )

    def handle_nonexistent_path(self, rel_path, tracked_files):
        matched = False
        for tracked_path in tracked_files:
            if (tracked_path == rel_path or
                    tracked_path.startswith(rel_path + os.sep)):
                matched = True
                self.__remove_if_tracked(tracked_path, None)
        if not matched:
            print(f"\033[91m'{rel_path}' is not tracked\033[0m")

    def __remove_if_tracked(self, rel_path, abs_path):
        if self.__index_handler.contains(rel_path):
            if abs_path and self.__path_handler.exists(abs_path):
                self.__path_handler.remove_file(abs_path)
            self.__index_handler.remove(rel_path)
        else:
            print(f"\033[91m'{rel_path}' is not tracked\033[0m")
